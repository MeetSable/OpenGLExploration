...
Texture::Texture(const std::string& filePath, TextureType type)
	:m_RendererID(0), m_FilePath(filePath), m_Width(0), m_Height(0), m_BPP(0), m_type(type)
{

	stbi_set_flip_vertically_on_load(1);
	unsigned char* texBuffer = stbi_load(filePath.c_str(), &m_Width, &m_Height, &m_BPP, 4);
	...
	GLCall(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, m_Width, m_Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, texBuffer));
